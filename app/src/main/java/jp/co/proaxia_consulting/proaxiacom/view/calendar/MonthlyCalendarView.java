...
public class MonthlyCalendarView extends View {
    ...
    //イベント入力用テキスト s.tanaka a start
    protected Paint eventText = new Paint(Paint.SUBPIXEL_TEXT_FLAG|Paint.ANTI_ALIAS_FLAG);
    protected FontMetrics metricsForE;
    //イベント入力用テキスト s.tanaka a end
    ...
    private void initResource(AttributeSet attrs) {
        ...
        //イベント入力用テキスト s.tanaka a start
        this.eventText.setColor(this.c_saturday);
        //イベント入力用テキスト s.tanaka a end
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //イベント入力用テキスト s.tanaka a start
        this.eventText.setStyle(Style.FILL);
        this.eventText.setTextSize(this.cellHeight * 0.17f); //テキストサイズはセルの高さの17%
        this.eventText.setTextScaleX(this.cellWidth / this.cellHeight); //セルのアスペクト比
        this.eventText.setTextAlign(Paint.Align.LEFT);
        this.metricsForE = eventText.getFontMetrics();
        //イベント入力用テキスト s.tanaka a end
...
//                    }
                    ...
                    //祭日名を非表示 s.tanaka c end

                    //DateInfoで設定したイベントフラグを参照し、フラグONであればイベントをセットする。 s.tanaka a start
                    if ( this.matrix[row-1][col].isEvent ) {
                        canvas.drawText( this.matrix[row-1][col].eventName
                                , ( col * this.cellWidth ) + holidayAlignX
                                , offsetY + holidayAlignY + alignY // ( row * this.cellHeight ) + hy + y
                                , this.eventText);
                    }
                    ///DateInfoで設定したイベントフラグを参照し、フラグONであればイベントをセットする。 s.tanaka a end
    ...
    /**
     * calendarを設定します(イベント入力後用) s.tanaka
     * @param calendar calendarをセットします
     */
    public void setCalendarDraw(Calendar calendar,int day) {
        this.calendar = calendar;
        this.setToDay(day);
        this.calcCalendarMatrix();
    }
